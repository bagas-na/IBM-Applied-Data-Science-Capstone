...
def get_scatter_plot(entered_site, max_min_range):
    filtered_df = spacex_df
    if entered_site == 'ALL':
        fig = px.scatter(spacex_df[(spacex_df["Payload Mass (kg)"] >= max_min_range[0])
                                   & (spacex_df["Payload Mass (kg)"] <= max_min_range[1])],
        ...
        fig = px.scatter(spacex_df[(spacex_df["Launch Site"] == entered_site)
                                   & (spacex_df["Payload Mass (kg)"] >= max_min_range[0])
                                   & (spacex_df["Payload Mass (kg)"] <= max_min_range[1])],
                         x="Payload Mass (kg)", y="class",
                         color='Booster Version Category', hover_data=['class'],
                         symbol="Booster Version Category",
